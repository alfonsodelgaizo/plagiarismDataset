...
## -->>> python3 generateClusterCnt.py NomeAlgoritmo TipoEmbedding <<<----
...
def generateCSV():
    ...
    with open('./Results_1_General/'+algo+'/coupleCnt_'+embedding+'.csv', 'w') as csvfile:
...
for j in range(0,len(coupleArray)):
   ...
   folderRead('./Results_1/'+algo+'/'+embedding+'/',song1,song2)
   i=i+1
...
sys.argv = ['./plottingTest.py',algo,embedding]
exec(open("./plottingTest.py").read())